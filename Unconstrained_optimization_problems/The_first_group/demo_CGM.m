...
for i=1:np %np
    [name,n,a]=init(i);
    [T1,NFF1,NGG1,NI1,N1] = CGMWWP(i,para1,'LS');
    [T2,NFF2,NGG2,NI2,N2] = CGMWWP(i,para1,'HS'); 
    [T3,NFF3,NGG3,NI3,N3] = CGMWWP(i,para1,'PRP');
    [T4,NFF4,NGG4,NI4,N4] = CGMWWP(i,para1,'FHTTCGM_PRP');
    [T5,NFF5,NGG5,NI5,N5] = CGMWWP(i,para1,'FHTTCGM_HS');
    [T6,NFF6,NGG6,NI6,N6] = CGMWWP(i,para1,'FHTTCGM_LS');

    T(i,:) = [T1,T2,T3,T4,T5,T6];
    NF(i,:) = [NFF1,NFF2,NFF3,NFF4,NFF5,NFF6];
    NG(i,:) = [NGG1,NGG2,NGG3,NGG4,NGG5,NGG6];
    NI(i,:) = [NI1,NI2,NI3,NI4,NI5,NI6];
    Ng(i,:) = [N1,N2,N3,N4,N5,N6];

    fprintf(fid_tex,'%s/%d & %d/%.3f/%.2e & %d/%.3f/%.2e & %d/%.3f/%.2e & %d/%.3f/%.2e & %d/%.3f/%.2e & %d/%.3f/%.2e\\\\ \r\n',...
                name,n,NI1,T1,N1,NI2,T2,N2,NI3,T3,N3,NI4,T4,N4,NI5,T5,N5,NI6,T6,N6);
end
%% 关闭文件
fclose(fid_tex);
...
legend('LS','HS','PRP','FHTTCGM_PRP','FHTTCGM_HS','FHTTCGM_LS','Location','southeast');     %修改成这样，在最后加一个0，自动找位置，下面的figure(2)~figure(5)同理
%subplot(2,2,2);
figure(2);
perf(NF,'logplot');
%title('目标函数计算性能');
...
legend('LS','HS','PRP','FHTTCGM_PRP','FHTTCGM_HS','FHTTCGM_LS','Location','southeast');
%subplot(2,2,3);
figure(3);
perf(NG,'logplot');
%title('梯度函数计算性能');
